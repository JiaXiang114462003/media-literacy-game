...
export default function GameScreen({
  ...
  const [latestVerifiedCardId, setLatestVerifiedCardId] = useState(null); // 追蹤最新被查證的卡片 ID
  const prevCardsState = useRef([]); // 追蹤上一次的卡片狀態
  ...
  // 追蹤最新被查證的卡片
  useEffect(() => {
    // 比較當前卡片狀態與上一次狀態，找出從 verifying 變為 isVerified 的卡片
    const prevCards = prevCardsState.current;

    // 找出剛剛從 verifying 變為 isVerified 的卡片
    for (const currentCard of newsCards) {
      const prevCard = prevCards.find((card) => card.id === currentCard.id);

      // 如果卡片從 verifying 變為 isVerified，使用異步方式更新最新被查證的卡片 ID
      if (
        prevCard &&
        prevCard.status === "verifying" &&
        currentCard.isVerified &&
        currentCard.status !== "verifying"
      ) {
        // 使用 queueMicrotask 異步設置 state，避免同步設置導致的級聯渲染
        queueMicrotask(() => {
          setLatestVerifiedCardId(currentCard.id);
        });
        break; // 只追蹤最新的一個
      }
    }
    ...
    // 更新上一次的卡片狀態
    prevCardsState.current = newsCards.map((card) => ({
      id: card.id,
      status: card.status,
    ...
    <div className="game-screen">
      <Layout round={round} trust={trust} fans={fans} barStyle={barStyle}>
        ...
        <div style={{ position: "relative", width: "100%", height: "100%" }}>
          {newsCards.map((news) => {
            // 計算 z-index：查證中的卡片最高，最新被查證的次之
            let zIndex = 1;
            if (news.status === CARD_STATUS.verifying) {
              zIndex = 2000; // 查證中的卡片置頂
            } else if (news.isVerified && news.id === latestVerifiedCardId) {
              zIndex = 1500; // 最新被查證的卡片置頂
            }
